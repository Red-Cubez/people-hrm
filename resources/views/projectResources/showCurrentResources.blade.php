<div class="panel-body">
    ...
    @if (count($projectResources)>0 )
        <div class="panel panel-default">
            <div class="panel-heading">Current Resources</div>
            <div class="panel-body">
                <table class="table table-striped task-table">
                    ...
                    @foreach ($projectResources as $projectResource)
                        <tr>
                            <td class="table-text">
                                <div>{{ $projectResource->resourceName}}</div>
                            </td>
                            <td>
                                ...
                                <form action="{{ url('projectresources/'.$projectResource->resourceId) }}"
                                    ...
                                    {{ csrf_field() }}
                                    {{ method_field('DELETE') }}
                                    <button class="btn btn-danger" data-toggle="confirmation" data-singleton="true"
                                            type="submit">
